...
const QuizTab: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
              ...
              className={`w-full p-4 rounded-xl text-left font-bold transition-all transform hover:scale-105
                ${matched.includes(item.id) 
                  ...
                  : selectedTerm === item.id 
                    ? 'bg-blue-50 border-2 border-blue-600 text-blue-700 shadow-md' 
            ...
            <button
              ...
              className={`w-full p-4 rounded-xl text-left text-sm transition-all
                ${matched.includes(item.id)
                  ? 'bg-gray-200 text-gray-400 border-2 border-transparent'
                  : selectedTerm 
                    ? 'bg-white border-2 border-dashed border-gray-300 hover:border-blue-400 hover:bg-blue-50 cursor-pointer animate-pulse'
      ...
      {isComplete && (
        <div className="mt-10 p-6 bg-blue-50 border border-blue-100 rounded-xl text-center animate-bounce">
          <h3 className="text-2xl font-bold text-blue-800">🎉 축하합니다! 모든 용어를 마스터하셨네요!</h3>
          <p className="text-blue-700 mt-2">이제 여러분은 스마트 팜 전문가입니다.</p>
        </div>
      )}
    </div>
  );
