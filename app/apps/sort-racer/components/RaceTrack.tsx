...
const RaceTrack: React.FC<RaceTrackProps> = ({ algorithmName, state, colorTheme, isWinner }) => {
  ...
  return (
    <div className={`relative flex flex-col h-full rounded-xl border ${themeColors.border} ${themeColors.bg} backdrop-blur-sm overflow-hidden transition-all duration-300 ${isWinner ? 'ring-4 ring-yellow-400 shadow-[0_0_30px_rgba(250,204,21,0.3)]' : ''}`}>
      {/* Header Stats */}
      <div className="p-4 border-b border-slate-700 bg-slate-900/50 flex justify-between items-center">
        <div>
          <h2 className={`text-lg font-bold ${themeColors.text}`}>{algorithmName}</h2>
          {finished && <span className="text-xs text-emerald-400 font-bold px-2 py-0.5 bg-emerald-900/30 rounded-full">완료!</span>}
        </div>
        <div className="flex space-x-4 text-sm font-mono">
          <div className="flex flex-col items-end">
            <span className="text-slate-400 text-xs flex items-center"><RefreshCw className="w-3 h-3 mr-1"/> 비교</span>
            <span className="text-white font-bold">{comparisons}</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-slate-400 text-xs flex items-center"><TrendingUp className="w-3 h-3 mr-1"/> 교환</span>
            <span className="text-white font-bold">{swaps}</span>
          </div>
        </div>
      </div>
      ...
      {isWinner && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="bg-black/60 backdrop-blur-sm p-4 rounded-xl border border-yellow-500 text-yellow-400 font-bold text-2xl animate-bounce">
                WINNER! 🏆
            </div>
        </div>
      )}
