  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-bold text-white mb-4">정렬 알고리즘, 왜 중요할까?</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 hover:border-indigo-500 transition-colors">
            ...
            <h3 className="text-xl font-bold text-white mb-2">비교 (Comparison)</h3>
        ...
        <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 hover:border-indigo-500 transition-colors">
            <div className="w-12 h-12 bg-red-900/50 rounded-lg flex items-center justify-center mb-4">
                <RefreshCw className="text-red-400 w-6 h-6" /> {/* Using RefreshCw as generic cycle/swap icon */}
            </div>
            <h3 className="text-xl font-bold text-white mb-2">교환 (Swap)</h3>
            <p className="text-slate-400 text-sm">
        ...
        <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 hover:border-indigo-500 transition-colors">
            ...
            <h3 className="text-xl font-bold text-white mb-2">시간 복잡도 (Big O)</h3>
            <p className="text-slate-400 text-sm">
      ...
      <div>
        <h3 className="text-2xl font-bold text-white mb-6">Big O 카드</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {BIG_O_CARDS.map((card, idx) => (
                <div key={idx} className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-2xl border border-slate-700 shadow-lg relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                        <span className="text-6xl font-black text-white">{card.title.replace('O(', '').replace(')', '')}</span>
                    </div>
                    <div className="relative z-10">
                        <h4 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400 mb-1">{card.title}</h4>
                        ...
                        <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800">
                            <span className="text-xs font-bold text-indigo-400 uppercase">Analogy</span>
                            <p className="text-slate-400 text-xs mt-1 italic">"{card.analogy}"</p>
                        </div>
                    </div>
                </div>
            ))}
