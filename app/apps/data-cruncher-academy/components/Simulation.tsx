...
export const Simulation: React.FC<SimulationProps> = ({ onTaskComplete }) => {
  ...
  const chartData = useMemo(() => {
    ...
    return [
      ...
      { name: '전송 시간(초)', value: result.transferTimeCompressed, type: '압축 후', color: '#2563eb' },
    ];
  }, [result]);
        ...
        <div className="space-y-3">
          {files.map(file => (
            <div
              ...
              className="group cursor-grab active:cursor-grabbing p-3 bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-300 rounded-lg transition-all flex items-center gap-3"
            >
      ...
      <div className="lg:col-span-6 flex flex-col gap-6">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
          <label className="block text-sm font-medium text-slate-700 mb-2 flex justify-between">
            ...
            <span className="text-blue-600 font-bold">{compressionLevel}%</span>
          </label>
          <input
            type="range"
            min="10"
      ...
      <div className="lg:col-span-3 bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col">
        ...
        {!result ? (
          ...
          <div className="animate-fade-in space-y-4">
            {/* Status Banner */}
            <div className={`p-3 rounded-lg flex items-start gap-3 ${result.success ? 'bg-blue-50 text-blue-900' : 'bg-red-50 text-red-900'}`}>
