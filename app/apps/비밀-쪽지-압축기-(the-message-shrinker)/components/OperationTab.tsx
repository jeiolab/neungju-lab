    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl border-l-4 border-green-500 shadow-sm">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-xl font-bold text-slate-800">작전 개시: 메시지 압축 프로토콜</h2>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
            <label className="block text-sm font-semibold text-slate-700 mb-2">비밀 메시지 입력</label>
            <textarea
              value={input}
              onChange={(e) => setInput(e.target.value.replace(/\s/g, ''))} // Removing spaces for simple RLE demo
              className="w-full h-32 bg-slate-50 border border-slate-300 rounded-lg p-3 text-slate-800 focus:ring-2 focus:ring-green-500 focus:outline-none font-mono"
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
            <label className="block text-sm font-semibold text-slate-700 mb-3">압축 도구 선택</label>
            ...
            <div className="mt-3 text-sm text-slate-600 p-3 bg-slate-50 rounded-lg border border-slate-200">
              {algorithm === AlgorithmType.RLE ? (
                <span><strong className="text-green-600">RLE:</strong> 연속된 문자를 묶어서 표현합니다. (예: AA -&gt; A2)</span>
              ) : (
                <span><strong className="text-blue-600">LZ:</strong> 이전에 나온 단어의 위치를 가리킵니다. (예: 학교...학교 -&gt; &lt;5,2&gt;)</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {result && (
            <>
              ...
              <div className={`p-5 rounded-xl border-2 ${result.isEfficient ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50'}`}>
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-bold flex items-center gap-2 text-slate-800">
                    <Activity size={20} />
                    압축 효율 분석
                  </h3>
                  <span className={`text-2xl font-mono font-bold ${result.ratio > 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {result.ratio}%
                  </span>
                </div>
                
                {!result.isEfficient && (
                  <div className="flex items-start gap-2 text-red-700 text-sm bg-red-100 p-2 rounded-lg mb-4">
                ...
                <div className="relative pt-6 pb-2">
                  ...
                  <div className="h-4 bg-slate-200 rounded-full overflow-hidden flex">
                    <div className="bg-slate-400 h-full transition-all duration-500" style={{ width: '100%' }}></div>
                  </div>
                  <div className="mt-2 h-4 bg-slate-200 rounded-full overflow-hidden flex relative">
                     <div 
              ...
              {/* Visualization Steps */}
              <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm h-80 overflow-y-auto">
                <h3 className="text-slate-700 font-bold mb-3 border-b border-slate-200 pb-2">처리 로그</h3>
                <div className="font-mono text-sm space-y-2">
                  <div className="flex items-center gap-2 text-slate-600">
                    <span>INPUT:</span>
                    <span className="text-slate-800 bg-slate-100 px-2 py-0.5 rounded break-all">{result.original}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-green-600 mb-4">
                    <span>OUTPUT:</span>
                    <span className="text-slate-800 bg-green-50 px-2 py-0.5 rounded border border-green-200 break-all">{result.compressed}</span>
                  </div>
