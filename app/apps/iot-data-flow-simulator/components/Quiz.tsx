  ...
  return (
    <div className="mt-12 w-full max-w-2xl mx-auto">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-2xl font-bold text-slate-900">퀴즈 풀기</h2>
      </div>

      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
        <div className="flex justify-between items-center mb-6">
            <span className="text-xs font-bold text-blue-600 tracking-wider uppercase">문제 {currentQuestionIdx + 1} / {QUIZ_DATA.length}</span>
            <div className="flex gap-1">
                {QUIZ_DATA.map((_, i) => (
                    <div key={i} className={`h-1.5 w-6 rounded-full ${i === currentQuestionIdx ? 'bg-blue-600' : 'bg-slate-200'}`}></div>
                ))}
            </div>
        </div>
        ...
        <AnimatePresence>
          {showExplanation && (
            ...
            >
              ...
              <button 
                onClick={handleNext}
                className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 rounded-lg flex items-center justify-center gap-2 transition-colors shadow-sm"
              >
