    ...
    <div className="max-w-4xl mx-auto p-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center">
          ...
          <div className="text-3xl font-mono font-bold text-blue-600">{score}</div>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center">
      ...
      {!isPlaying ? (
        <div className="text-center bg-white p-12 rounded-xl border-2 border-dashed border-slate-300">
          ...
          <button
            onClick={startGame}
            className="bg-blue-600 text-white px-8 py-3 rounded-lg text-lg font-bold hover:bg-blue-700 transition-transform active:scale-95 flex items-center gap-2 mx-auto"
          >
        ...
        <div className="space-y-6">
           {/* Active Game Area */}
           <div className="bg-slate-50 border border-slate-200 text-blue-600 p-6 rounded-xl text-center shadow-sm font-mono relative overflow-hidden">
             <div className="absolute top-0 left-0 w-full h-1 bg-blue-100">
               <div className="h-full bg-blue-600 transition-all duration-1000 ease-linear" style={{ width: `${(timeLeft/60)*100}%` }}></div>
             </div>
             <div className="text-sm text-blue-500 mb-1">DECRYPT TARGET</div>
             <div className="text-5xl font-bold tracking-widest text-slate-900">{target}</div>
           </div>
          ...
          <div className="text-center font-mono text-xl text-slate-600">
             Current: <span className={`${currentSum === target ? 'text-blue-600' : 'text-slate-900'} font-bold`}>{currentSum}</span>
          </div>
        </div>
      )}
    </div>
