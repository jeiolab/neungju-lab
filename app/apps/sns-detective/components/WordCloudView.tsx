...
export const WordCloudView: React.FC<WordCloudViewProps> = ({ words }) => {
  ...
  useEffect(() => {
    ...
    const simulation = d3.forceSimulation<Node>(data)
      ...
      .force("collide", d3.forceCollide<Node>().radius((d: Node) => d.r + 2).iterations(2));
    ...
    const nodes = svg.append("g")
      ...
      .on("mouseover", function(this: SVGElement) {
          d3.select(this).select("text").transition().attr("font-size", (d: any) => (d as any).size * 1.2);
      })
      .on("mouseout", function(this: SVGElement) {
          d3.select(this).select("text").transition().attr("font-size", (d: any) => (d as any).size);
      });
