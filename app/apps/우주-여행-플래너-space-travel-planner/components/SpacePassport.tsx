...
const SpacePassport: React.FC<SpacePassportProps> = ({ isOpen, onClose, visitedRecords }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-[#1a1d2e] w-full max-w-2xl rounded-xl border-2 border-yellow-600 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="bg-yellow-700 p-4 flex justify-between items-center relative">
          <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/leather.png')]"></div>
          <h2 className="text-2xl font-bold text-yellow-100 flex items-center gap-3 relative z-10">
            <Stamp className="w-6 h-6" />
            우주 여권 (Space Passport)
          </h2>
          <button onClick={onClose} className="text-yellow-100 hover:text-white relative z-10">
            <X className="w-6 h-6" />
          </button>
        </div>

        {/* Content */}
        <div className="p-8 overflow-y-auto bg-[#eaddcf]">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
            {PLANETS.map((planet) => {
              ...
              return (
                <div key={planet.id} className="aspect-[3/4] border-2 border-dashed border-yellow-800/30 rounded-lg flex flex-col items-center justify-center p-2 relative group bg-[#f3efe7]">
                  ...
                  {isVisited ? (
                    ...
                    <div className="opacity-10 grayscale flex flex-col items-center">
                       ...
                       <span className="text-gray-400 text-xs">미방문</span>
                    </div>
                  )}
                </div>
              );
          ...
          <div className="mt-8 border-t border-yellow-800/20 pt-4 text-center">
            <p className="text-yellow-900 font-serif italic text-sm">
              "탐험은 인간의 본성이다." - NASA
            </p>
            <p className="text-yellow-800/60 text-xs mt-1">
