  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div className="text-center py-4">
        <h2 className="text-2xl font-bold text-slate-800">디지털 융합 직업 만들기</h2>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-3">
            {TECHS.map((item) => (
              ...
              >
                {renderIcon(item.iconName, `w-5 h-5 ${selectedTech === item.id ? 'text-blue-600' : 'text-slate-400'}`)}
                <span className={`text-sm font-medium ${selectedTech === item.id ? 'text-blue-700' : 'text-slate-600'}`}>{item.label}</span>
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="bg-white rounded-3xl overflow-hidden shadow-2xl border border-slate-100 max-w-2xl mx-auto">
            ...
            <div className="bg-blue-600 text-white p-8 text-center relative overflow-hidden">
              <div className="relative z-10 flex flex-col items-center">
                <div className="w-20 h-20 bg-white rounded-2xl rotate-3 flex items-center justify-center mb-6 shadow-lg text-blue-600">
                  {renderIcon(result.iconName, "w-10 h-10")}
                </div>
                <h3 className="text-3xl font-bold mb-2 text-white">{result.title}</h3>
                <div className="flex gap-2 text-xs text-blue-100 uppercase tracking-widest mt-2">
