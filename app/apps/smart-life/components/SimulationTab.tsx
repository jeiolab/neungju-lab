        ...
        <div className="p-6 bg-gray-50 border-b flex justify-between items-center">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-sm font-bold text-blue-600">{totalScore}점 / 90점</span>
            </div>
          </div>
          <button 
            onClick={handleReset}
        ...
        <div className="flex-1 bg-[url('https://images.unsplash.com/photo-1586023492125-27b2c045efd7?q=80&w=2000&auto=format&fit=crop')] bg-cover bg-center relative group">
          ...
          <div className="absolute inset-0 p-8 grid grid-cols-2 gap-4 place-content-center">
             ...
             {placedDevices.map((device) => (
               <div key={device.id} className="bg-white/90 p-4 rounded-xl shadow-lg border-2 border-blue-100 flex items-center gap-3 animate-bounce-in">
                 <div className="p-2 bg-blue-100 rounded-full text-blue-600">
                   {getDeviceIcon(device.iconName)}
                 </div>
                 <div>
                   <p className="font-bold text-gray-800 text-sm">{device.name}</p>
                   <p className="text-xs text-blue-600 font-semibold">+편리함 {device.score}</p>
                 </div>
               </div>
             ))}
          </div>
          ...
          {showPopup && lastActionMessage && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 animate-slide-up w-max max-w-[90%]">
              <CheckCircle className="w-5 h-5 text-green-100 shrink-0" />
