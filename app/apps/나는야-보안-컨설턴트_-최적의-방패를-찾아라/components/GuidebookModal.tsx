...
const GuidebookModal: React.FC<GuidebookModalProps> = ({ score, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-slate-800 w-full max-w-2xl max-h-[80vh] rounded-xl shadow-2xl border border-slate-600 flex flex-col">
        <div className="p-4 border-b border-slate-700 flex justify-between items-center bg-slate-900 rounded-t-xl">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            <BookOpen className="text-blue-400" /> 나만의 보안 가이드북
          </h2>
        ...
        <div className="p-6 overflow-y-auto flex-grow custom-scrollbar">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-slate-300">AI가 당신의 활동을 분석하여 요약집을 작성 중입니다...</p>
            </div>
          ) : (
        ...
        <div className="p-4 border-t border-slate-700 bg-slate-900 rounded-b-xl text-right">
            <button onClick={onClose} className="bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded transition-colors">
                닫기
            </button>
        </div>
      </div>
