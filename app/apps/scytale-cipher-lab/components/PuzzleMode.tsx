  ...
  return (
    <div className="flex flex-col items-center space-y-8 animate-fadeIn max-w-2xl mx-auto">
      <div className="text-center space-y-2">
        <h2 className="text-2xl font-bold text-slate-900">암호 해독가의 도전</h2>
        <p className="text-slate-600">타일을 재배열하여 숨겨진 메시지를 찾아보세요.</p>
      </div>
      ...
      <div className="flex gap-4">
        ...
        <button
          ...
          disabled={isLoading}
          className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-500 transition-colors shadow-sm disabled:opacity-50"
        >
          {isLoading ? (
      ...
      <div className="bg-slate-50 p-8 rounded-xl border border-slate-200 relative">
        {isSolved && (
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-white/90 backdrop-blur-sm rounded-xl animate-fadeIn">
            <div className="text-center transform scale-110">
              <CheckCircle className="w-16 h-16 text-green-500 mx-auto mb-2" />
              <h3 className="text-2xl font-bold text-slate-900">해독 성공!</h3>
              <p className="text-blue-600 font-bold text-lg mt-2">{targetMessage}</p>
            </div>
          </div>
        )}
        ...
        >
          {tiles.map((tile) => (
            <div
              ...
              className={`
                aspect-square flex items-center justify-center text-2xl font-bold rounded-lg shadow-sm cursor-grab active:cursor-grabbing
                transition-all duration-200 select-none
                ${isSolved ? 'bg-green-100 text-green-800 border-2 border-green-500' : 'bg-white text-slate-900 border-b-4 border-blue-600 hover:-translate-y-1'}
