    ...
    <div className="p-4 bg-slate-900 border border-cyan-500/30 rounded-lg shadow-xl text-cyan-50">
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          <input 
            ...
            className="flex-1 bg-slate-800 border border-slate-600 p-2 rounded text-white font-mono focus:border-cyan-500 outline-none"
            onKeyDown={(e) => e.key === 'Enter' && checkHash()}
          />
          <button 
            onClick={checkHash}
            className="bg-cyan-700 hover:bg-cyan-600 p-2 rounded text-white"
          >
      ...
      {result && (
        <div className="animate-pulse">
          <label className="block text-xs uppercase text-slate-400 mb-1">생성된 해시 (MD5)</label>
          <div className="w-full bg-black border border-red-500/50 p-2 rounded text-red-400 font-mono text-sm break-all">
            {result}
          </div>
        </div>
      )}
      ...
      <div className="mt-4 pt-4 border-t border-slate-800">
        <h4 className="text-xs uppercase text-slate-500 mb-2">알려진 유출 비밀번호 (힌트)</h4>
