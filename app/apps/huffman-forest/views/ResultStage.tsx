      ...
      {(node.left || node.right) && (
        <div className="flex items-start justify-center mt-12 relative w-full">
           ...
           <svg className="absolute top-[-3rem] left-0 w-full h-12 pointer-events-none z-0" style={{ overflow: 'visible' }}>
             {/* Main Stem */}
             <line x1="50%" y1="0" x2="50%" y2="50%" stroke="#475569" strokeWidth="3" />
             {/* Horizontal Bar */}
             <line x1="25%" y1="50%" x2="75%" y2="50%" stroke="#475569" strokeWidth="3" strokeLinecap="round" />
             {/* Left Leg */}
             {node.left && <line x1="25%" y1="50%" x2="25%" y2="100%" stroke="#475569" strokeWidth="3" strokeLinecap="round" />}
             {/* Right Leg */}
             {node.right && <line x1="75%" y1="50%" x2="75%" y2="100%" stroke="#475569" strokeWidth="3" strokeLinecap="round" />}
           </svg>
              ...
              {node.left && (
                <>
                  ...
                  <div className="absolute top-[-1.8rem] left-[50%] -translate-x-1/2 z-20">
                     <div className="bg-blue-100 text-blue-700 text-sm font-bold w-6 h-7 flex items-center justify-center rounded-md border-2 border-blue-300 shadow-sm">
                       0
                     </div>
                  </div>
            ...
            <div className="flex-1 flex justify-center relative">
              {node.right && (
                <>
                  ...
                  <div className="absolute top-[-1.8rem] left-[50%] -translate-x-1/2 z-20">
                     <div className="bg-slate-100 text-slate-700 text-sm font-bold w-6 h-7 flex items-center justify-center rounded-md border-2 border-slate-300 shadow-sm">
                       1
                     </div>
                  </div>
  ...
  return (
    <div className="w-full max-w-7xl flex flex-col lg:flex-row gap-6 pb-12 items-start">
      ...
      <div className="flex-[2] w-full flex flex-col gap-6">
        ...
        {/* Tree Container */}
        <div className="bg-white rounded-xl border border-slate-200 p-6 md:p-8 shadow-sm overflow-x-auto relative min-h-[400px]">
          <div className="absolute top-4 left-4 bg-blue-100 px-3 py-1 rounded-full text-blue-700 font-bold text-sm z-30 border border-blue-200">
             🌲 완성된 코드 나무
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {Object.entries(codeMap).map(([char, code]) => (
            <motion.div 
              ...
              className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center hover:border-blue-300 transition-colors"
            >
              <span className="font-extrabold text-slate-900 text-xl">{char}</span>
              <span className="font-mono text-blue-600 bg-blue-50 px-2 py-0.5 rounded text-sm mt-1 border border-blue-200">{code}</span>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 w-full flex flex-col gap-6 sticky top-4">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="h-40 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} layout="vertical" barSize={20}>
                ...
                <Bar dataKey="bits" radius={[0, 10, 10, 0]} background={{ fill: '#f1f5f9', radius: [0, 10, 10, 0] } as any}>
                   <LabelList dataKey="bits" position="right" fill="#64748b" fontSize={12} formatter={(val: any) => `${val} bit`} />
          ...
          <div className="bg-blue-50 rounded-xl p-3 text-center mt-2 border border-blue-200">
            <span className="text-slate-700 text-sm">총 크기가</span>
            <strong className="text-2xl text-blue-600 mx-2">{compressionRatio}%</strong>
            <span className="text-slate-700 text-sm">줄어들었어요!</span>
          </div>
        </div>

        {/* Quiz Section */}
        <div className="bg-gradient-to-br from-blue-600 to-blue-700 p-6 rounded-xl shadow-md text-white relative overflow-hidden border border-blue-800">
          ...
          <div className="relative mb-4">
            <input 
              ...
              className="w-full pl-4 pr-10 py-3 rounded-xl text-slate-900 font-mono text-lg placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white"
            />
          </div>
          ...
          {quizResult === null && (
             <button 
               ...
               className="w-full bg-amber-500 hover:bg-amber-400 text-white font-bold py-3 rounded-xl transition-all shadow-md active:scale-95"
             >
               정답 확인하기
             </button>
          )}
          ...
          {quizResult === 'correct' && (
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white/20 p-4 rounded-xl backdrop-blur-sm border border-white/30">
              ...
              <p className="text-sm text-blue-100">허프만 코딩의 원리를 완벽하게 이해하셨네요!</p>
            </motion.div>
          )}
        ...
        <Button variant="secondary" onClick={onReset} className="w-full py-4 bg-white hover:bg-slate-50 !text-slate-700 !border-slate-200">
