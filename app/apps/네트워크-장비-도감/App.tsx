...
const App: React.FC = () => {
  ...
  const renderHome = () => (
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 text-center max-w-4xl mx-auto">
      ...
      <h1 className="text-4xl md:text-6xl font-black text-slate-900 mb-6 tracking-tight">
        Net<span className="text-blue-600">Runner</span>
      </h1>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <button 
            ...
            className="flex items-center gap-2 font-black text-xl tracking-tight text-slate-900 hover:text-blue-600 transition-colors"
          >
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 px-6 py-3 flex justify-between items-center z-50 safe-area-bottom">
        <button onClick={() => setCurrentView(AppView.HOME)} className={`flex flex-col items-center gap-1 ${currentView === AppView.HOME ? 'text-blue-600' : 'text-slate-400'}`}>
          <Home size={20} />
          <span className="text-[10px] font-bold">홈</span>
        </button>
        <button onClick={() => setCurrentView(AppView.LEARN)} className={`flex flex-col items-center gap-1 ${currentView === AppView.LEARN ? 'text-blue-600' : 'text-slate-400'}`}>
          <BookOpen size={20} />
          <span className="text-[10px] font-bold">도감</span>
        </button>
        <button onClick={() => setCurrentView(AppView.SIMULATION)} className={`flex flex-col items-center gap-1 ${currentView === AppView.SIMULATION ? 'text-blue-600' : 'text-slate-400'}`}>
          <PlayCircle size={20} />
          <span className="text-[10px] font-bold">게임</span>
        </button>
        <button onClick={() => setCurrentView(AppView.SUMMARY)} className={`flex flex-col items-center gap-1 ${currentView === AppView.SUMMARY ? 'text-blue-600' : 'text-slate-400'}`}>
          <Info size={20} />
