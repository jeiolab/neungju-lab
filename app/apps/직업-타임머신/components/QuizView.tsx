    ...
    <div className="flex flex-col items-center justify-center min-h-[500px] animate-fade-in p-2">
      <div className="w-full max-w-lg bg-white rounded-3xl shadow-xl overflow-hidden border border-purple-100">
        <div className="bg-purple-600 p-6 text-white text-center">
          ...
          <p className="text-purple-200 text-sm mt-1">미래 기술 용어를 맞춰보세요 ({currentStep + 1}/{QUIZ_DATA.length})</p>
        </div>
        ...
        <div className="p-8 flex flex-col items-center">
          ...
          {!showHint && !result && (
             <button 
              ...
              className="text-gray-400 text-xs underline mb-6 hover:text-purple-600"
             >
               힌트 보기
             </button>
          )}
          ...
          {result === 'correct' ? (
            <div className="flex flex-col items-center animate-bounce-in">
              ...
              <button 
                onClick={handleNext}
                className="flex items-center space-x-2 bg-purple-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-purple-700 transition-colors"
              >
                <span>다음 문제</span>
                <ArrowRight size={20} />
              </button>
            ...
            <form onSubmit={handleSubmit} className="w-full">
              <div className="relative mb-6">
                <input
                  ...
                  className={`w-full text-center text-xl p-4 border-2 rounded-xl focus:outline-none focus:ring-4 transition-all
                    ${result === 'wrong' ? 'border-red-300 focus:ring-red-100 bg-red-50' : 'border-gray-200 focus:border-purple-500 focus:ring-purple-100'}
                  `}
                  autoFocus
                />
                {result === 'wrong' && (
              ...
              <button
                type="submit"
                className="w-full bg-gray-900 text-white py-4 rounded-xl font-bold text-lg hover:bg-gray-800 transition-colors shadow-lg active:scale-95 transform duration-100"
              >
                정답 확인
              </button>
              {result === 'wrong' && (
