  ...
  if (!started) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] bg-white rounded-2xl shadow-xl p-8 text-center">
        ...
        <button 
          onClick={startQuiz}
          className="bg-slate-900 text-white px-10 py-4 rounded-full font-bold text-lg hover:bg-slate-800 shadow-lg hover:scale-105 transition-all"
        >
          도전 시작!
        </button>
      </div>
    ...
    return (
      <div className="bg-white rounded-2xl shadow-xl p-8 text-center min-h-[400px] flex flex-col items-center justify-center">
        ...
        <button 
          ...
          className="bg-amber-500 text-white px-8 py-3 rounded-lg font-bold hover:bg-amber-600"
        >
          메인으로 돌아가기
        </button>
      </div>
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 border-b-8 border-slate-200 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="w-24 h-24 bg-slate-100 rounded-full flex items-center justify-center mb-4 border-4 border-slate-900">
        ...
        <div className="grid gap-3">
          {currentQ.options.map((option, idx) => (
            <button
              ...
              className="w-full text-left p-4 rounded-xl border-2 border-slate-100 hover:border-amber-500 hover:bg-amber-50 transition-all font-medium text-slate-700 group flex items-center justify-between"
            >
              <span>{idx + 1}. {option}</span>
              <ArrowRight className="opacity-0 group-hover:opacity-100 text-amber-500 transition-opacity" size={16} />
            </button>
          ))}
        </div>
      </div>
