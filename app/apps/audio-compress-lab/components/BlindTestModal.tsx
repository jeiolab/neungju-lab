...
const BlindTestModal: React.FC<BlindTestModalProps> = ({ isOpen, onClose, setTempMode, playAudio, stopAudio }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white w-full max-w-md rounded-2xl border border-slate-200 shadow-xl overflow-hidden relative">
        ...
        <div className="p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">오디오 블라인드 테스트</h2>
            <p className="text-slate-600 text-sm mb-8">당신의 귀는 CD 원음과 강한 압축 음원의 차이를 구분할 수 있나요?</p>

            {stage === 'intro' && (
                <button 
                    onClick={startTest}
                    className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold transition-all shadow-sm"
                >
                    듣기 시작
                </button>
            )}
            ...
            {stage === 'playing' && (
                <div className="animate-fade-in">
                    <div className="flex items-center justify-center gap-2 mb-8 text-emerald-600 font-mono text-sm animate-pulse">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <button 
                            ...
                            className="p-4 bg-white border-2 border-slate-200 hover:border-emerald-500 hover:bg-emerald-50 rounded-lg text-slate-900 font-medium transition-all"
                        >
                            원본 (무손실)
                        </button>
                        <button 
                            onClick={() => handleGuess('low_quality')}
                            className="p-4 bg-white border-2 border-slate-200 hover:border-amber-500 hover:bg-amber-50 rounded-lg text-slate-900 font-medium transition-all"
                        >
                            압축본 (저음질)
                        </button>
                    </div>
            ...
            {stage === 'result' && (
                <div className="animate-fade-in">
                    <div className={`text-4xl font-bold mb-4 ${result === 'correct' ? 'text-emerald-600' : 'text-red-600'}`}>
                    ...
                    <button 
                        onClick={reset}
                        className="px-6 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-white text-sm shadow-sm"
                    >
                        다시 시도하기
                    </button>
                </div>
