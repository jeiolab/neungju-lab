...
const AudioVisualizer: React.FC<AudioVisualizerProps> = ({ analyser, compressionMode, isPlaying }) => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        if (isCut) {
            ...
            // Active Bar
            // Gradient based on frequency - adjusted for white background
            const hue = (i / bufferLength) * 360 + 200; // Blue to Pink range
            ctx.fillStyle = `hsl(${hue}, 70%, 50%)`; // Slightly darker for white bg
        }
  ...
  return (
    <div className="relative w-full h-64 bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
      ...
      {/* Legend Overlay */}
      <div className="absolute top-2 right-2 flex flex-col gap-1 pointer-events-none bg-white/90 backdrop-blur-sm border border-slate-200 p-2 rounded text-xs shadow-sm">
