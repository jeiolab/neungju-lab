...
const InfoPanel: React.FC<InfoPanelProps> = ({ originalStats, compressedStats, onDownload, quality }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col gap-6">
      ...
      <button 
        onClick={onDownload}
        className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-medium flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md hover:shadow-lg"
      >
        <Download size={18} />
        압축된 이미지 다운로드
      </button>
